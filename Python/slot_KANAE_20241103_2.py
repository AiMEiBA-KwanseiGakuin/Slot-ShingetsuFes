# made by 金栄智治 (ほぼ2024/11/3新月祭にて動作していたもの)
import numpy as np
import pygame,sys
...
class Slot(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        #'''os.path.join(os.getcwd(),"pictures",name+".jpeg")
    ...
    def draw(self,screen):# 画像描画
        '''
        draw=[[ # ゴミのような可読性
            screen.blit(self.images[self.real[i][(self.real_pos[i]+j)%self.num]],(i*310,(j_*150)+20))
        for j_,j in enumerate([1,0,-1]) ] for i in range(3) ]
        '''
...
def main():
    ...
    while True:
        ...
        myslot.draw_7seg(screen,370,490,myslot.count)
        if tick%tickrate==0:
            tick=0
            ## ゲーム処理 ##
            if myslot.state==Opening:#   待機状態
                ...
                myslot.choose_role()
                myslot.finish()
                if myslot.count>myslot.highest:
                    myslot.highest=myslot.count
