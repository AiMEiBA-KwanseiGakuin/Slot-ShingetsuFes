# made by 金栄智治 (自己満足保存用(動かなかったらslot_KANAE.pyを参照))
import numpy as np
import pygame,sys
from pygame.locals import *
import serial

Arduino_used=False# テスト用　本番ではTrue
...
class Slot:
    rad=np.random.default_rng()# 乱数生成用
    def __init__(self):
        self.tile_num = 21         #リールのタイル数
        self.tile_size = 150,300
        self.seg_size = 135,200
        ...
        self.p_table = np.concatenate([p_list,[1-np.sum(p_list)]])# 確率テーブル
        ...
        self.stopping=["","",""]# top/mid/btm
        
        # ↓もろもろの状態変数など↓
        self.can_start=False   # 開始条件
        self.state=Opening     # 状態変数
        self.flag=None         # 事前抽選するフラグ用(set_flagで使用予定)
        self.bonus=False       # ボーナス状態(未実装)
        self.score=0           # スコア
        self.highest=0         # 最高スコア記録
        self.game_count=5      # 一人当たりのゲーム回数

    def start(self):# ゲーム開始時に呼び出し
        if self.can_start and self.game_count>0:
            self.real_run=[1,1,1]
            self.set_flag()
            self.state=Running
    ...
    def choose_role(self):# 全リール停止後(slidestop内でも使うかも?)の役判定
        result=np.array([
        [self.left[self.real_pos[0]-1%self.tile_num], self.center[self.real_pos[1]-1%self.tile_num], self.right[self.real_pos[2]-1%self.tile_num]],
        [self.left[self.real_pos[0]], self.center[self.real_pos[1]], self.right[self.real_pos[2]]],
        [self.left[(self.real_pos[0]+1)%self.tile_num], self.center[(self.real_pos[1]+1)%self.tile_num], self.right[(self.real_pos[2]+1)%self.tile_num]]])
        # ↓可読性0バージョン↓
        result_=np.array([[self.real[i][(self.real_pos[i]+j)%self.tile_num] for i in range(3)] for j in [-1,0,1]])
    ...
    def cashback(self,val,seven=0):# 払い戻し
        #self.d_score=val#7セグ表示用のやつ 描画時にd_score=0になるまで1フレームづつscore+=1する予定
        self.score+=1#val
        if seven:self.score+=1
        print(f"そろえた数:{self.score}")
...
def main():
    ...
    while True:
        ...
        screen.fill((0,0,0))
        myslot.draw(screen)
        myslot.draw_7seg(screen,780,490,myslot.game_count)
        #myslot.draw_7seg(screen,20,490,difficulty)
        myslot.draw_7seg(screen,370,490,myslot.score)
        if tick%tickrate==0:
            tick=0
            ## ゲーム処理 ##
            if myslot.state==Opening:#   待機状態
                ...
                myslot.choose_role()
                myslot.finish()
                if myslot.score>myslot.highest:
                    myslot.highest=myslot.score
                    print(f"NEW RECORD {myslot.highest}!!")
        ...
        if Arduino_used:
            ...
            if button==b'left\r\n':
                ...
                myslot.stop(Center)
            elif button==b'right\r\ n':
                myslot.stop(Right)
            elif button==b'start\r\n' and myslot.state==Opening:
                myslot.can_start=True
